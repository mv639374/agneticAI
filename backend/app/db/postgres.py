...
async def get_db_session() -> AsyncGenerator[AsyncSession, None]:
    ...
    This is a dependency injection pattern for FastAPI endpoints and internal use.
    It ensures:
    1. Session is created for each request/operation
    ...
    Usage in internal code (like MCP tools):
        async with get_db_session() as db:
            result = await db.execute(select(Conversation))
            conversations = result.scalars().all()
    
    Yields:
        AsyncSession: Database session for this request/operation
    """
